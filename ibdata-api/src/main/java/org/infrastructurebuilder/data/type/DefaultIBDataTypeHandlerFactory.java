...
public class DefaultIBDataTypeHandlerFactory implements IBDataTypeHandler {
  ...
  public DefaultIBDataTypeHandlerFactory(IBRuntimeUtils ibr, List<IBDataType> dataTypes,
    ...
    this(requireNonNull(dataTypes).stream().sorted(weighted).collect(toList()),
        translators.stream().sorted(weighted).collect(toList()), requireNonNull(ibr));
  }
  ...
  public IBDataTypeHandler configure(ConfigMapSupplier cms) {
    ...
    JSONObject qq = new JSONObject(cms.get());
    List<IBDataType> d = dataTypes.stream().map(v -> v.configure(qq)).collect(toList());
    this.ibr.getLog().log(Level.INFO, "Loaded " + dataTypes.size() + " IBData types");
    StringJoiner sj = new StringJoiner(",");
    d.stream().map(IBDataType::getType).forEach(sj::add);
    this.ibr.getLog().log(Level.DEBUG, " IBDataTypes > " + sj.toString());
