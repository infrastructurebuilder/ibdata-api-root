...
import static org.infrastructurebuilder.util.core.IBUtils.nullSafeUUIDComparator;
...
import org.infrastructurebuilder.util.credentials.basic.URLAndCreds;;
...
public interface IBDataStreamIdentifier extends ChecksumEnabled {
  public final static Comparator<IBDataStreamIdentifier> ibDataStreamComparator = Comparator
      ...
      // Check Date
      .thenComparing(IBDataStreamIdentifier::getCreationDate, nullSafeInstantComparator);
  ...
  default Checksum getChecksum() {
    return ofNullable(getSha512()).filter(s -> s.length() == 128) // Length of a sha512
        .map(org.infrastructurebuilder.util.core.Checksum::new)
  ...
  default Checksum getMetadataChecksum() {
    return ChecksumBuilder.newInstance()
        ...
        .addLong(getNumRows())
        // orig len
        .addLong(getOriginalLength())
        // orig row count
        .addLong(getOriginalRowCount())
        // URL
        .addString(getUrl())
        //
        .asChecksum();
  ...
  default Optional<Long> getInputStreamLength() {
    return Optional.of(getOriginalLength());
  }
   ...
   * @return
   */
  Optional<Long> getOriginalRowCount();
