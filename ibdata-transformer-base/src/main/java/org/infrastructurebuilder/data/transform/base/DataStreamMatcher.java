...
public class DataStreamMatcher extends DataStream {
  ...
  boolean matches(IBDataStreamIdentifier ds) {
    return Optional.ofNullable(ds).map(d -> {
      ...
      retVal &= matchString(getUuid(), Optional.ofNullable(d.getId()).map(UUID::toString).orElse(null));
      retVal &= matchDate(Instant.parse(Objects.requireNonNull(getCreationDate())), d.getCreationDate());
      retVal &= matchString(getName(), d.getName());
      retVal &= matchString(getDescription(), d.getDescription().orElse(null));
      return retVal;
    }).orElse(false);
  }

  private boolean matchDate(Instant creationDate2, Instant creationDate) {
